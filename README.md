...
这是一个只读监控程序，不下单。一个常驻进程同时监控三个子账户，持续保存成交 JSONL，并按固定间隔自动完成配对、权益/收益计算、Excel/HTML 生成和邮件发送。
...
报告位于 `output/<account_id>/YYYYMMDD/`。旧的测试、重放和报告命令仍保留，供故障排查和历史数据补算使用，但日常运行不需要它们。

Excel/HTML 的收益分布沿用 C++ 定义：实际损益是 09:30 基准权益到当前权益的差值；交易损益来自配对成交；费率损益来自 Binance 资金费率历史；波动损益来自 09:30 基准持仓与当前标记价格的变化；理论损益 = 交易损益 + 费率损益 + 波动损益；损益差值用于校验实际与理论的偏差。HK/US 市场成交量、挂单率、采样次数等 Binance 当前未采集的指标暂不填入，避免伪精确结果。
