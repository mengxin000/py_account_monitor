...
- 最多每秒推送一次最新快照，底层行情采集频率不变，慢客户端超时断开；行情超过3秒无更新标记过期。
- 每500ms检查JSONL尾部，新成交增量处理，乱序重建。无变化复用结果，不重建Excel。分页列表每页50条，仅在账户、页码或计算版本变化后请求（每2秒兜底检查）。
- 网页使用协议v2：同一条WebSocket内切换账户/交易对；只传变化的数据分区，不随盘口重复发送成交原文。最多一个未确认快照，客户端处理并确认后才发送最新状态，慢链路不排队补发历史盘口。上限1次/秒，实际频率受往返耗时影响；30秒未确认断线重连。旧客户端仍兼容原快照协议。
